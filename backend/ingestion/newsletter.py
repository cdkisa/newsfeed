    ...
    async def fetch(self, source: Source) -> list[RawContent]:
        ...
        for entry in feed.entries:
            ...
            if source.last_fetched_at and published:
                ...
                if pub_aware <= cutoff:
                    continue
            results.append(RawContent(title=entry.get("title", "Untitled"), url=entry.get("link", ""), body=entry.get("summary", ""), published_at=published, source_type=SourceType.newsletter))
        return results
